...
import 'dart:io';
import 'package:hbe/distributor/screens/return/add_return.dart';
import 'package:hbe/distributor/screens/return/update_return.dart';
...
class _ReturnDetailState extends State<ReturnDetail> {
  ...
  Future<List<ReturnModel>>? dailySale;
  ...
  void initState() {
    ...
    if(mounted){
      setState(() {
        dailySale = getReturnSales();
      });
    }
  }

  //GET Return SALE BY USER_ID API
  Future<List<ReturnModel>> getReturnSales() async {

    try {
      final response = await http.get(
          Uri.parse('${ApiUrls.baseURL}${ApiUrls.getSaleDetailsR}?UserID=${globalData.userId}'),
          headers: {
      ...
      if (response.statusCode == 200&& data[0]["PO_Number"].toString() != "null") {
        ...
        return data.map((data) => ReturnModel.fromJson(data)).toList();
      }
  ...
  Future<void> deleteSale(selectedID) async{
    try {
      final response = await http.post(
          Uri.parse('${ApiUrls.baseURL}${ApiUrls.deleteSaleR}?SaleReturnID=$selectedID&UserID=${globalData.userId}'),
          headers: {
            ApiUrls.key_name: ApiUrls.apikey,
          });
  ...
  Future<void> postSale(selectedID) async{
    try {
      final response = await http.post(
          Uri.parse('${ApiUrls.baseURL}${ApiUrls.postSaleR}?SaleReturnID=$selectedID&UserID=${globalData.userId}'),
          headers: {
            ApiUrls.key_name: ApiUrls.apikey,
          });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("Sale Return",style: TextStyle(fontFamily: "Poppins-Medium",fontSize: 16),),
        centerTitle: true,
      ),
          ...
          Center(
            child: Column(
              children: [
                ...
                Text("No Return Sale Found",style: TextStyle(fontFamily:'Poppins-SemiBold',fontSize: 20),)
              ],
            ),
          ): FutureBuilder<List<ReturnModel>>(
            future: dailySale,
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                List<ReturnModel>? data = snapshot.data;
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context,index){
                      return buildListCard(context,data![index],(){
                        AppRoutes.push(context, PageTransitionType.fade, UpdateReturn(returnModel: data[index]));
                      });
      ...
      floatingActionButton: FloatingActionButton(
        backgroundColor: greenBasic,
        onPressed: (){
          AppRoutes.push(context, PageTransitionType.fade, AddReturn());
        },
  ...
  Widget buildListCard(BuildContext context, ReturnModel dailySaleModel,onTap) {
    return Container(
            ...
            Row(
              ...
              children: [
                SizedBox(
                  ...
                  child: Text(dailySaleModel.cusCustomerName.toString(),style: TextStyle(fontFamily: 'Poppins-SemiBold',fontSize: 16,color: greenBasic),),
                ),
                Text("${dailySaleModel.totalAmount.toStringAsFixed(0)} PKR",style: TextStyle(fontFamily: 'Poppins-SemiBold',fontSize: 16,color: black))
              ],
            ),
            SizedBox(height: 10,),
            Row(
              ...
              children: [
                InkWell(
                  onTap: (){
                    ...
                    deleteSale(dailySaleModel.saleReturnId.toString());
                ...
                InkWell(
                  onTap: (){
                    ...
                    postSale(dailySaleModel.saleReturnId.toString());
