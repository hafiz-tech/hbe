...
class DistributorChangePassword extends StatefulWidget {
  const DistributorChangePassword({Key? key}) : super(key: key);

  @override
  State<DistributorChangePassword> createState() => _DistributorChangePasswordState();
}

class _DistributorChangePasswordState extends State<DistributorChangePassword> {
  ...
  //CHANGE PASSWORD API
  Future<void> DistributorChangePassword() async {

    try {
      var response = await http.post(
        Uri.parse(
            '${ApiUrls.distributorUrl}${ApiUrls.resetPassword2}?UserID=${globalData.userId}&OldPassword=${Uri.encodeComponent(currentPasswordController.text.toString())}&NewPassword=${Uri.encodeComponent(newPasswordController.text.toString())}'),
        headers: {
