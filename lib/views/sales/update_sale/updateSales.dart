...
class UpdateSales extends StatefulWidget {
  ...
  const UpdateSales({Key? key,required this.dailySaleModel}) : super(key: key);
...
class _UpdateSalesState extends State<UpdateSales> {
  ...
  Future<void> getProductsForUpdate() async {
    ...
    try {
      log('${ApiUrls.baseURL}${ApiUrls.getSaleDetails_Single2}?UserID=${globalData.userId}&SaleID=${widget.dailySaleModel.poid}');
      final response = await http.get(
          Uri.parse('${ApiUrls.baseURL}${ApiUrls.getSaleDetails_Single2}?UserID=${globalData.userId}&SaleID=${widget.dailySaleModel.poid}'),
          headers: {
  ...
  Future<void> updateMasterSale() async{
    try {
      ...
      final response = await http.post(
          Uri.parse('${ApiUrls.baseURL}${ApiUrls.updateMasterSale}?POID=${widget.dailySaleModel.poid}&PODate=${widget.dailySaleModel.poDate}&CustomerID=${widget.dailySaleModel.cusCustomerId}&Remarks=${remarksController.text.toString()}&UserID=${globalData.userId}'),
          headers: {
            ApiUrls.key_name: ApiUrls.apikey,
          });
  ...
  Future<void> deleteSale(poID) async{
    try {
      log('${ApiUrls.baseURL}${ApiUrls.deleteDetSale}?POID=$poID');
      final response = await http.post(
          Uri.parse('${ApiUrls.baseURL}${ApiUrls.deleteDetSale}?POID=$poID'),
          headers: {
            ApiUrls.key_name: ApiUrls.apikey,
          });
  ...
  Future<void> submitDetailSale(String poID,String productID, int quantity) async{
    ...
    try {
      log('${ApiUrls.baseURL}${ApiUrls.submitDetSale}?POID=${widget.dailySaleModel.poid}&ProductID=$productID&CustomerId=${widget.dailySaleModel.cusCustomerId}&Quantity=$quantity');
      final response = await http.post(
          Uri.parse('${ApiUrls.baseURL}${ApiUrls.submitDetSale}?POID=${widget.dailySaleModel.poid}&ProductID=$productID&CustomerId=${widget.dailySaleModel.cusCustomerId}&Quantity=$quantity'),
          headers: {
            ApiUrls.key_name: ApiUrls.apikey,
          });
      ...
      if (response.statusCode == 200) {
        ...
        Future.delayed(Duration(seconds: 1),(){
          AppRoutes.pushAndRemoveUntil(context, DailySales());
        });
      }
      else {
                    ...
                    child: Column(
                      children: [
                        ...
                        CustomButton(
                            ...
                            }, color: greenBasic, text: "Update Daily Sale")
                      ],
                    ),
                  )
                ),
