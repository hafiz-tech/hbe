...
class UpdatePO extends StatefulWidget {
  final DailySaleModel dailySaleModel;
  const UpdatePO({Key? key,required this.dailySaleModel}) : super(key: key);

  @override
  State<UpdatePO> createState() => _UpdatePOState();
}

class _UpdatePOState extends State<UpdatePO> {
  ...
  Future<void> getProductsForUpdate() async {
    ...
    try {
      final response = await http.get(
          Uri.parse('${ApiUrls.baseURL}${ApiUrls.getPODetails_Single}?UserID=${globalData.userId}&POID=${widget.dailySaleModel.poid}'),
          headers: {
  ...
  //SUBMIT MASTER PO
  Future<void> updateMasterPO() async{
    try {
      log('${ApiUrls.baseURL}${ApiUrls.updateMasterPO}?POID=${widget.dailySaleModel.poid}&PODate=${widget.dailySaleModel.poDate}&CustomerID=${widget.dailySaleModel.cusCustomerId}&Remarks=${remarksController.text.toString()}&UserID=${globalData.userId}');
      final response = await http.post(
          Uri.parse('${ApiUrls.baseURL}${ApiUrls.updateMasterPO}?POID=${widget.dailySaleModel.poid}&PODate=${widget.dailySaleModel.poDate}&CustomerID=${widget.dailySaleModel.cusCustomerId}&Remarks=${remarksController.text.toString()}&UserID=${globalData.userId}'),
          headers: {
            ApiUrls.key_name: ApiUrls.apikey,
          });
      ...
      if (response.statusCode == 200&& res.toString()=="Update" ) {
        ToastUtils.successToast("Updating PO...", context);
        ...
        Future.delayed(Duration(milliseconds: 1500),(){
          for (int i = 0; i < getUpdateProducts.length; i++) {
            ...
            if (detailMethodCount < getUpdateProducts.length) {
              deletePO(widget.dailySaleModel.poid);
            }
            else{
              if(mounted){
                setState(() {
            ...
            Future.delayed(Duration(milliseconds: 1500),(){
              for (int i = 0; i < getUpdateProducts.length; i++) {
                ...
                if (detailMethodCount3 < getUpdateProducts.length) {
                  submitDetailPO(widget.dailySaleModel.poid.toString(),getUpdateProducts[i].productId.toString(),double.parse(getUpdateProducts[i].quantity.toString()).toInt(),false);
                }
                else{
                  submitDetailPO(widget.dailySaleModel.poid.toString(),getUpdateProducts[i].productId.toString(),double.parse(getUpdateProducts[i].quantity.toString()).toInt(),true);
  ...
  //DELETE PO
  Future<void> deletePO(poID) async{
    try {
      log('${ApiUrls.baseURL}${ApiUrls.deleteDetPO}?POID=$poID');
      final response = await http.post(
          Uri.parse('${ApiUrls.baseURL}${ApiUrls.deleteDetPO}?POID=$poID'),
          headers: {
            ApiUrls.key_name: ApiUrls.apikey,
          });
  ...
  //SUBMIT DETAIL API
  Future<void> submitDetailPO(String poID,String productID, int quantity, bool isLast) async{

    try {
      log('${ApiUrls.baseURL}${ApiUrls.submitDetPO}?POID=${widget.dailySaleModel.poid}&ProductID=$productID&CustomerId=${widget.dailySaleModel.cusCustomerId}&Quantity=$quantity&IslastEntry=$isLast');
      final response = await http.post(
          Uri.parse('${ApiUrls.baseURL}${ApiUrls.submitDetPO}?POID=${widget.dailySaleModel.poid}&ProductID=$productID&CustomerId=${widget.dailySaleModel.cusCustomerId}&Quantity=$quantity&IslastEntry=$isLast'),
          headers: {
            ApiUrls.key_name: ApiUrls.apikey,
          });
      ...
      if (response.statusCode == 200) {
        ...
        if(isLast){
          log("COMING HERE");
          // startSecondLoop(customerID.toString());
          ToastUtils.successToast("Purchase Order updated successfully!", context);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("Update Purchase Order",style: TextStyle(fontFamily: "Poppins-Medium",fontSize: 16),),
        centerTitle: true,
      ),
                    ...
                    child: Column(
                      children: [
                        ...
                        CustomButton(
                            ...
                            }, color: greenBasic, text: "Update Purchase Order")
                      ],
                    ),
                  )
                ),
