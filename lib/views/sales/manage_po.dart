...
import 'package:hbe/views/sales/add_po_invoice.dart';
import 'package:hbe/views/sales/update_po/update_PO.dart';
...
class ManagePO extends StatefulWidget {
  const ManagePO({Key? key}) : super(key: key);

  @override
  State<ManagePO> createState() => _ManagePOState();
}

class _ManagePOState extends State<ManagePO> {
  bool isLoading = true;
  bool showOther = false;
  Future<List<DailySaleModel>>? dailyPO;
  ...
  void initState() {
    ...
    if(mounted){
      setState(() {
        dailyPO = getPODetails();
      });
    }
  }

  //GET DAILY SALE BY USER_ID API
  Future<List<DailySaleModel>> getPODetails() async {

    try {
      final response = await http.get(
          Uri.parse('${ApiUrls.baseURL}${ApiUrls.getPODetails}?UserID=${globalData.userId}'),
          headers: {
  ...
  Future<void> deleteSale(selectedID) async{
    try {
      final response = await http.post(
          Uri.parse('${ApiUrls.baseURL}${ApiUrls.deletePO}?POID=$selectedID&UserID=${globalData.userId}'),
          headers: {
            ApiUrls.key_name: ApiUrls.apikey,
          });
  ...
  Future<void> postSale(selectedID) async{
    try {
      final response = await http.post(
          Uri.parse('${ApiUrls.baseURL}${ApiUrls.postPO}?POID=$selectedID&UserID=${globalData.userId}'),
          headers: {
            ApiUrls.key_name: ApiUrls.apikey,
          });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("Manage PO",style: TextStyle(fontFamily: "Poppins-Medium",fontSize: 16),),
        centerTitle: true,
      ),
      drawer: DrawerWidget(),
      body: LoadingAnimation(
        ...
        child: Padding(
          ...
          Center(
            child: Column(
              children: [
                Lottie.asset("assets/animations/no_po.json",height: 300),
                Text("No PO Invoice Found",style: TextStyle(fontFamily:'Poppins-SemiBold',fontSize: 20),)
              ],
            ),
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context,index){
                      return  buildListCard(context,data![index], (){
                        AppRoutes.push(context, PageTransitionType.fade, UpdatePO(dailySaleModel: data![index]));
                      });
      ...
      floatingActionButton: FloatingActionButton(
        backgroundColor: greenBasic,
        onPressed: (){
          AppRoutes.push(context, PageTransitionType.fade, AddPOInvoice());
        },
