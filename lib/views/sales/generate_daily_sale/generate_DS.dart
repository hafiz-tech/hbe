...
class GenerateDS extends StatefulWidget {
  final String cID,date;
  const GenerateDS({Key? key,required this.cID,required this.date}) : super(key: key);

  @override
  State<GenerateDS> createState() => _GenerateDSState();
}

class _GenerateDSState extends State<GenerateDS> {
  ...
  Future<void> submitMasterSale() async{
    try {
      final response = await http.post(
          Uri.parse('${ApiUrls.baseURL}${ApiUrls.submitMstrSale}?PODate=${widget.date}&CustomerID=${widget.cID}&Remarks=${remarksController.text.toString()}&UserID=${globalData.userId}'),
          headers: {
            ApiUrls.key_name: ApiUrls.apikey,
          });
  ...
  Future<void> submitDetailPO(String poID, String quantity) async{
    ...
    try {
      final response = await http.post(
          Uri.parse('${ApiUrls.baseURL}${ApiUrls.submitDetSale}?POID=$invoiceID&ProductID=$poID&CustomerId=${widget.cID}&Quantity=$quantity'),
          headers: {
            ApiUrls.key_name: ApiUrls.apikey,
          });
